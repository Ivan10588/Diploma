...
from equipment.models import ChatMessage
...
def chat_view(request, equipment_id=None, user_id=None):
    ...
    if chat_type == 'general':
        ...
        unread = messages_list.filter(sender=other_user, is_read=False)
    ...
    if request.method == 'POST':
        ...
        if chat_type == 'general':
            ...
            target_url = (
                reverse('equipment:chat_for_equipment', kwargs={'equipment_id': equipment.id})
                if equipment else
                reverse('equipment:chat_general', kwargs={'user_id': other_user.id})
            )
